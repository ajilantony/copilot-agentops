...
5. **Suggest Community Resources** (if copilot-agentops MCP available)
...
**Requires: `copilot-agentops` MCP server**

If the `mcp_copilot-agentops_search_instructions` or `mcp_copilot-agentops_load_collection` tools are available, use them to suggest relevant community resources:

1. **Detect Available MCP Tools**
   - Check if `mcp_copilot-agentops_*` tools are accessible
   - If NOT available, skip this functionality entirely and inform user they can enable it by adding the copilot-agentops MCP server

2. **Search for Relevant Resources**
   - Use `mcp_copilot-agentops_search_instructions` with keywords from detected stack
   - Query for: language name, framework, common patterns (e.g., "typescript", "react", "testing", "mcp")

3. **Suggest Collections**
   - Use `mcp_copilot-agentops_list_collections` to find curated collections
...
4. **Load and Install**
   - Use `mcp_copilot-agentops_load_collection` to fetch collection details
...
Searching copilot-agentops for relevant resources...
...
**Important:** Only suggest copilot-agentops resources when the MCP tools are detected. Do not hallucinate tool availability.
...
These are the official requirements from copilot-agentops. The agent does NOT deep-validate every file, but uses these when generating templates:
...
6. **Check MCP Availability** - Before suggesting copilot-agentops resources, verify that `mcp_copilot-agentops_*` tools are available. If not present, do NOT suggest or reference these tools. Simply skip the community resource suggestions.

## MCP Tool Detection

Before using copilot-agentops features, check for these tools:

```
Available MCP tools to check:
- mcp_copilot-agentops_search_instructions
- mcp_copilot-agentops_load_instruction
- mcp_copilot-agentops_list_collections
- mcp_copilot-agentops_load_collection
```

**If tools are NOT available:**
- Skip all `/suggest` functionality
- Do not mention copilot-agentops collections
- Focus only on local scaffolding
- Optionally inform user: "Enable the copilot-agentops MCP server for community resource suggestions"
