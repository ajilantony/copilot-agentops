---
agent: 'agent'
description: 'Suggest relevant GitHub Copilot instruction files from the copilot-agentops repository based on current repository context and chat history, avoiding duplicates with existing instructions in this repository, and identifying outdated instructions that need updates.'
tools: ['edit', 'search', 'runCommands', 'runTasks', 'think', 'changes', 'testFailure', 'openSimpleBrowser', 'web/fetch', 'githubRepo', 'todos', 'search']
---
# Suggest Copilot AgentOps Instructions

Analyze current repository context and suggest relevant copilot-instruction files from the [GitHub copilot-agentops repository](https://github.com/github/copilot-agentops/blob/main/docs/README.instructions.md) that are not already available in this repository.

## Process

1. **Fetch Available Instructions**: Extract instruction list and descriptions from [copilot-agentops README.instructions.md](https://github.com/github/copilot-agentops/blob/main/docs/README.instructions.md). Must use `#fetch` tool.
...
4. **Fetch Remote Versions**: For each local instruction, fetch the corresponding version from copilot-agentops repository using raw GitHub URLs (e.g., `https://raw.githubusercontent.com/github/copilot-agentops/main/instructions/<filename>`)
...
11. **Output**: Provide structured table with suggestions, descriptions, and links to both copilot-agentops instructions and similar local instructions
...
12. **Download/Update Assets**: For requested instructions, automatically:
    ...
    - Update outdated instructions by replacing with latest version from copilot-agentops
...
Display analysis results in structured table comparing copilot-agentops instructions with existing repository instructions:

| copilot-agentops Instruction | Description | Already Installed | Similar Local Instruction | Suggestion Rationale |
|------------------------------|-------------|-------------------|---------------------------|---------------------|
| [blazor.instructions.md](https://github.com/github/copilot-agentops/blob/main/instructions/blazor.instructions.md) | Blazor development guidelines | ✅ Yes | blazor.instructions.md | Already covered by existing Blazor instructions |
| [reactjs.instructions.md](https://github.com/github/copilot-agentops/blob/main/instructions/reactjs.instructions.md) | ReactJS development standards | ❌ No | None | Would enhance React development with established patterns |
| [java.instructions.md](https://github.com/github/copilot-agentops/blob/main/instructions/java.instructions.md) | Java development best practices | ⚠️ Outdated | java.instructions.md | applyTo pattern differs: remote uses `'**/*.java'` vs local `'*.java'` - Update recommended |
...
1. For each local instruction file, construct the raw GitHub URL to fetch the remote version:
   - Pattern: `https://raw.githubusercontent.com/github/copilot-agentops/main/instructions/<filename>`
...
Instructions files in copilot-agentops use this front matter format:
```markdown
---
...
- Use `githubRepo` tool to get content from copilot-agentops repository instructions folder
...
- Include links to both copilot-agentops instructions and similar local instructions
