---
agent: 'agent'
description: 'Suggest relevant GitHub Copilot collections from the copilot-agentops repository based on current repository context and chat history, providing automatic download and installation of collection assets, and identifying outdated collection assets that need updates.'
tools: ['edit', 'search', 'runCommands', 'runTasks', 'think', 'changes', 'testFailure', 'openSimpleBrowser', 'web/fetch', 'githubRepo', 'todos', 'search']
---
# Suggest Copilot AgentOps Collections

Analyze current repository context and suggest relevant collections from the [GitHub copilot-agentops repository](https://github.com/github/copilot-agentops/blob/main/docs/README.collections.md) that would enhance the development workflow for this repository.

## Process

1. **Fetch Available Collections**: Extract collection list and descriptions from [copilot-agentops README.collections.md](https://github.com/github/copilot-agentops/blob/main/docs/README.collections.md). Must use `#fetch` tool.
...
4. **Fetch Remote Versions**: For each local asset that matches a collection item, fetch the corresponding version from copilot-agentops repository using raw GitHub URLs (e.g., `https://raw.githubusercontent.com/github/copilot-agentops/main/<type>/<filename>`)
...
11. **Download/Update Assets**: For requested collections, automatically:
    ...
    - Update outdated assets by replacing with latest version from copilot-agentops
...
|-----------------|-------------|-------|---------------|---------------------|
| [Azure & Cloud Development](https://github.com/github/copilot-agentops/blob/main/collections/azure-cloud-development.md) | Comprehensive Azure cloud development tools including Infrastructure as Code, serverless functions, architecture patterns, and cost optimization | 15 items | 3 similar | Would enhance Azure development workflow with Bicep, Terraform, and cost optimization tools |
| [C# .NET Development](https://github.com/github/copilot-agentops/blob/main/collections/csharp-dotnet-development.md) | Essential prompts, instructions, and chat modes for C# and .NET development including testing, documentation, and best practices | 7 items | 2 similar | Already covered by existing .NET-related assets but includes advanced testing patterns |
| [Testing & Test Automation](https://github.com/github/copilot-agentops/blob/main/collections/testing-automation.md) | Comprehensive collection for writing tests, test automation, and test-driven development | 11 items | 1 similar | Could significantly improve testing practices with TDD guidance and automation tools |
...
1. For each local asset file that corresponds to a collection item, construct the raw GitHub URL:
   - Agents: `https://raw.githubusercontent.com/github/copilot-agentops/main/agents/<filename>`
   - Prompts: `https://raw.githubusercontent.com/github/copilot-agentops/main/prompts/<filename>`
   - Instructions: `https://raw.githubusercontent.com/github/copilot-agentops/main/instructions/<filename>`
...
1. **Fetch Collection Manifest**: Get collection YAML from copilot-agentops repository
...
- Use `fetch` tool to get collections data from copilot-agentops repository
...
- Include links to both copilot-agentops collections and individual assets within collections

## Collection Installation Workflow

1. **User Confirms Collection**: User selects specific collection(s) for installation
2. **Fetch Collection Manifest**: Download YAML manifest from copilot-agentops repository
...
- 🔄 Asset outdated (update available from copilot-agentops)
